...
const translations = {
  ru: {
    sc8Title: "А ТАКЖЕ, У НАС ИМЕЕТСЯ...",
    sc7Title: "РАЦИИ - РАДИОСТАНЦИИ",
    sc4Title: "Пожарные щиты и шкафы",
    sc5Title: "Система автоматического пожаро тушения",
    sc6Title: "Противопожарная сигнализация",
  ...
  uz: {
    sc8Title: "Bundan tashqari, bizda mavjud...",
    sc7Title: "Radiostansiyalar",
    sc4Title: "O't o'chirish shitlari va javonlari",
    sc5Title: "Avtomatik yong'in o'chirish tizimi",
    sc6Title: "Yong'in signalizatsiyasi",
...
// --- ФУНКЦИЯ ПЕРЕКЛЮЧЕНИЯ КНОПКИ ЦЕНЫ ---
function updatePriceButton(lang) {
  lang = lang || 'ru';

  const uzBtn = document.querySelector('a[href="https://t.me/ntauz"]');
  const cnBtn = document.querySelector('a[href="https://t.me/ntachina"]');

  if (lang === 'zh') {
    // Китайский — показываем китайскую кнопку, прячем узбекскую/русскую
    if (uzBtn) uzBtn.style.display = 'none';
    if (cnBtn) cnBtn.style.display = 'inline-block';
  } else {
    // Другой язык — показываем узбекскую кнопку, прячем китайскую
    if (uzBtn) uzBtn.style.display = 'inline-block';
...
options.forEach(option => {
  option.addEventListener('click', () => {
    ...
    applyTranslations(lang);
    updatePriceButton(lang); // 👈 ключевое исправление
  });
});

// Инициализация при первой загрузке
applyTranslations('ru');
updatePriceButton('ru');